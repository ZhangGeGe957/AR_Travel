...
class CommunityViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITextFieldDelegate, WaterfallMutiSectionDelegate, UIViewControllerTransitioningDelegate {
    ...
    private var detailsPage: CommunityDetailsPageViewController = CommunityDetailsPageViewController() // 详情页
    ...
    override func viewDidLoad() {
        ...
        LocationManager.shareManager.creatLocationManager().startLocation { curLocation, curAddress, curCity, errorReason in
            print("经度 \(curLocation?.coordinate.longitude ?? 0.0)")
            print("纬度 \(curLocation?.coordinate.latitude ?? 0.0)")
            print("地址 \(curAddress ?? "")")
            print("城市 \(curCity ?? "")")
            print("error \(errorReason ?? "没有错误")")
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if (indexPath.section == 0) {
            ...
            photoCell.photoImageView?.image = UIImage(named: self.communityModel.photoStringWithArray(indexPath.row))
            photoCell.titleLabel?.text = "23213123123"
            return photoCell
        }
    }
